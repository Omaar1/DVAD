...
# PATHS
$ShareRoot = "C:\vagrant\provisioners\services\SCCM\MECM_Setup"
...
if ($iniMismatch.Count -gt 0) {
    throw ("ConfigMgrAutoSave.ini is out of sync with lab-config.json:`n" + ($iniMismatch -join "`n") +
        "`nEdit provisioners/services/SCCM/MECM_Setup/ConfigMgrAutoSave.ini to match lab-config.json, then re-run.")
}
